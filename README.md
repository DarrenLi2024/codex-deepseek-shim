...
在 **Codex Desktop** 中使用 DeepSeek / Claude / OpenAI 等自定义模型，绕过官方模型白名单限制。只需两步命令即可在 DeepSeek 和官方 GPT-5.5 间自由切换。
...
```bash
# 配置好 ~/.factory/settings.json 后，只需记住两个命令：

codex-shim app .     # 启动 Codex Desktop → 走 DeepSeek
codex-shim quit      # 恢复到官方 GPT-5.5 通道
```

就是这么简单。`codex-shim app .` 会自动生成目录、启动代理、注入配置、打开 Codex。`codex-shim quit` 停止代理并恢复原始配置。在任何终端路径下都可执行。

---

## 项目简介

这是一个轻量本地 Python 代理服务器，伪装成 OpenAI Responses API 端点。Codex 将请求发到 shim，shim 根据配置转发给真实的上游 API（OpenAI Chat Completions / Anthropic Messages），同时完成协议转换和适配。
...
```bash
# 启动 Codex + DeepSeek（自动生成目录、启动 shim、注入配置）
codex-shim app .

# 切回官方 GPT-5.5（停止 shim、恢复原始配置）
codex-shim quit
```

`codex-shim app .` 执行的操作：
1. 自动生成模型目录和配置文件
2. 启动 shim 守护进程（后台，127.0.0.1:8765）
3. 向 `~/.codex/config.toml` 注入 shim 配置（原配置有备份）
4. 关闭当前 Codex，重新启动
...
```bash
# 设置默认模型为某个特定模型
codex-model deepseek-v4-pro       # 或 deepseek-v4-flash
codex-app                         # 重启 Codex

# 如果你希望 shim 持续运行但临时走 ChatGPT 官方通道：
codex-shim mode direct            # gpt-5.5 → ChatGPT
codex-shim restart
...
**问题**：上游的启动流程需要 `generate` → `start` → `app .` 三步，切换回官方模式也没有直观命令。

**解决方案**：`codex-shim app .` 一键完成全部启动操作。`codex-shim quit` 一键恢复官方模式。将使用门槛降到最低。

### 2. DeepSeek V4 Thinking 模式兼容
...
| 命令 | 说明 |
|---|---|
| **`codex-shim app .`** | **一键启动 Codex + DeepSeek（最常用）** |
| **`codex-shim quit`** | **一键恢复官方 GPT-5.5 通道（最常用）** |
| `codex-shim generate` | 重新生成模型目录和配置 |
| `codex-shim start` | 启动后台守护进程 |
| `codex-shim stop` | 停止守护进程（保留 shim 配置） |
| `codex-shim restart` | 重启守护进程 |
| `codex-shim status` | 健康检查 + 模型数量 |
| `codex-shim list` | 列出所有模型和路由 |
| `codex-shim mode [direct\|shim]` | 查看或切换路由模式 |
